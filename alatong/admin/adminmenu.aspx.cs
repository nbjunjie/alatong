    ...
    public partial class adminmenu : System.Web.UI.Page
    {
        ...
        private void DelData()
        {
            ...
            if (FunctionClass.CheckStr(strDel, 1))
            {
                ...
                myData.ExecuteSql("delete from T_AdminMenu where ID=" + strDel, myConn);
        ...
        {
            string strPage, strKeyWord, strPID, strSql;
            ...
            //初始化sql语句
            strSql = "select * from T_AdminMenu where 1=1";
            ...
            //判断NewType参数是否正确
            if (!FunctionClass.CheckStr(strPID, 1))
            {
            ...
            strSql += " order by SID asc,id;select * from T_AdminMenu where PID=0 order by SID,id";

            DataSet myDs = myData.GetDataSet(strSql, myConn);

            //赋值新闻分类下拉框
            ddlPID.Items.Add(new ListItem("所有菜单", "0"));
            myData.InitProTypeList(myDs.Tables[1], 0, ddlPID, "","title");
            ddlPID.SelectedValue = strPID;
            ...
            for (int i = 0; i < intPageSize; i++)
            {
                ...
                myTc2.Attributes.Add("onclick", "SetShowOrFalse(this,'6','" + myDs.Tables[0].Rows[intCurrentRow]["IsShow"].ToString()
                    + "','" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString() + "')");

                TableCell myTc8 = new TableCell();
                myTc8.Text = myDs.Tables[0].Rows[intCurrentRow]["PID"].ToString();

                TableCell myTc3 = new TableCell();
                myTc3.Text = myDs.Tables[0].Rows[intCurrentRow]["title"].ToString();

                TableCell myTc4 = new TableCell();
                myTc4.Text = myDs.Tables[0].Rows[intCurrentRow]["link"].ToString();
                ...
                myTc6.Width = 60;
                myTc6.Text = "<a href='adminmenu_mod.aspx?id=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
                    + "' style='color:#330099;'>修改</a> <a href='adminmenu.aspx?del=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
        ...
        {
            string strSql, strPID, strKeyWord;

            strKeyWord = Request.QueryString["KeyWord"];
            strPID = Request.QueryString["PID"];
            strSql = "select ID from T_AdminMenu where 1=1";
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    if (myTB.Checked)
                        myData.ExecuteSql("delete from T_AdminMenu where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
        ...
        void myTbOrder_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                if (myTB != null)
                {
                    myData.ExecuteSql("update T_AdminMenu set SID=" + myTB.Text + " where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
