    ...
    public partial class book : System.Web.UI.Page
    {
        ...
        private void DelData()
        {
            ...
            if (FunctionClass.CheckStr(strDel, 1))
            {
                ...
                myData.ExecuteSql("delete from T_Book where ID=" + strDel, myConn);
        ...
        {
            string strPage, strKeyWord, strSql, strIsRecommend;
            ...
            //初始化sql语句
            strSql = "select * from T_Book where 1=1 ";
            ...
            if (FunctionClass.CheckStr(strKeyWord, 0))
            {
                ...
                strSql += " and (Title like '%" + strKeyWord.Replace("'", "''") + "%' or Memo like '%" + strKeyWord.Replace("'", "''") + "%')";
            }
            ...
            for (int i = 0; i < intPageSize; i++)
            {
                ...
                if (myDs.Tables[0].Rows[intCurrentRow]["IsShow"].ToString() == "1")
                    myTc3.Text = "<FONT color=blue>√</FONT>";
                else
                    myTc3.Text = "<FONT color=red>×</FONT>";
                TableCell myTc2 = new TableCell();
                myTc2.Text = FunctionClass.WriteHtml(myDs.Tables[0].Rows[intCurrentRow]["Title"].ToString());
                //TableCell myTc11 = new TableCell();
                //myTc11.Text = FunctionClass.WriteHtml(myDs.Tables[0].Rows[intCurrentRow]["Tel"].ToString());
                //TableCell myTc111 = new TableCell();
                //myTc111.Text = FunctionClass.WriteHtml(myDs.Tables[0].Rows[intCurrentRow]["EMail"].ToString());

                TableCell myTc12 = new TableCell();
                myTc12.Text = FunctionClass.WriteHtml(myDs.Tables[0].Rows[intCurrentRow]["Memo"].ToString());
                ...
                //myTr.Cells.Add(myTc111);
                myTr.Cells.Add(myTc12);
                ...
                myTc6.Width = 60;
                myTc6.Text = "<a href='book_mod.aspx?id=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
                    + "' style='color:#330099;'>修改</a> <a href='book.aspx?del=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
        ...
        void myTbDel_Click(object sender, EventArgs e)
        {
            ...
            strKeyWord = Request.QueryString["KeyWord"];
            strSql = "select ID from T_Book where 1=1";
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    if (myTB.Checked)
                        myData.ExecuteSql("delete from T_Book where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
        ...
        {
            string strSql, strKeyWord, strTypeSql;
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                if (myTB != null)
                {
                    myData.ExecuteSql("update T_Book set SID=" + myTB.Text + " where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
