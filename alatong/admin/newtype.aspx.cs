...
{
    public partial class newtype : System.Web.UI.Page
    {
        ...
        private void DelData()
        {
            ...
            if (FunctionClass.CheckStr(strDel, 1))
            {
                ...
                myData.ExecuteSql("delete from T_NewType where ID=" + strDel, myConn);
        ...
        private void BindData()
        {
            ...
            //初始化sql语句
            strSql = "select * from T_NewType where 1=1";
            ...
            for (int i = 0; i < intPageSize; i++)
            {
                ...
                myTc3.Text = myDs.Tables[0].Rows[intCurrentRow]["PID"].ToString();

                TableCell myTc4 = new TableCell();
                myTc4.Text = myDs.Tables[0].Rows[intCurrentRow]["TypeCalled"].ToString();
                ...
                myTc6.Width = 60;
                myTc6.Text = "<a href='newtype_mod.aspx?id=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
                    + "' style='color:#330099;'>修改</a> <a href='newtype.aspx?del=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
        ...
        void myTbDel_Click(object sender, EventArgs e)
        {
            ...
            strKeyWord = Request.QueryString["KeyWord"];
            strSql = "select ID from T_NewType where 1=1";
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    if (myTB.Checked)
                        myData.ExecuteSql("delete from T_NewType where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
        ...
        void myTbOrder_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                if (myTB != null)
                {
                    myData.ExecuteSql("update T_NewType set SID=" + myTB.Text + " where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
