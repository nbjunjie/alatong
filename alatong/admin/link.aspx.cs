    ...
    public partial class link : System.Web.UI.Page
    {
        ...
        private void DelData()
        {
            ...
            if (FunctionClass.CheckStr(strDel, 1))
            {
                ...
                myData.ExecuteSql("delete from T_Link where ID=" + strDel, myConn);
        ...
        private void BindData()
        {
            ...
            //初始化sql语句
            strSql = "select * from V_Link where 1=1";
            ...
            strSql += " order by SID asc,id desc;select * from T_LinkType order by SID,ID desc";
            ...
            for (int i = 0; i < intPageSize; i++)
            {
                ...
                myTc2.Attributes.Add("onclick", "SetShowOrFalse(this,'7','" + myDs.Tables[0].Rows[intCurrentRow]["IsShow"].ToString()
                    + "','" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString() + "')");

                TableCell myTc8 = new TableCell();
                myTc8.Text = myDs.Tables[0].Rows[intCurrentRow]["Link"].ToString();
                ...
                myTc4.Text = myDs.Tables[0].Rows[intCurrentRow]["Called"].ToString();

                TableCell myTc5 = new TableCell();
                myTc5.Text = myDs.Tables[0].Rows[intCurrentRow]["NoteTime"].ToString();

                TableCell myTc10 = new TableCell();
                if (myDs.Tables[0].Rows[intCurrentRow]["IsShowOther"].ToString() == "1")
                    myTc10.Text = "<FONT color=blue>√</FONT>";
                else
                    myTc10.Text = "<FONT color=red>×</FONT>";
                myTc10.Attributes.Add("onclick", "SetShowOrFalse(this,'8','" + myDs.Tables[0].Rows[intCurrentRow]["IsShowOther"].ToString()
                    + "','" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString() + "')");
                ...
                myTc6.Width = 60;
                myTc6.Text = "<a href='link_mod.aspx?id=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
                    + "' style='color:#330099;'>修改</a> <a href='link.aspx?del=" + myDs.Tables[0].Rows[intCurrentRow]["ID"].ToString()
        ...
        void myTbDel_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < myDs.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    if (myTB.Checked)
                        myData.ExecuteSql("delete from T_Link where ID=" + strRowID, myConn);
                }
            }
            myDs.Dispose();
